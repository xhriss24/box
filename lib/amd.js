...
Sock.sendUi = async (
  ...
  try {
    ...
    let isVideo = false;
    if (!cachedUserImages || !cachedUserImages[0]) {
      cachedUserImages = global.userImages
        ? global.userImages.split(",")
        : [await botpic()];
      cachedUserImages = cachedUserImages.filter(
        (imageUrl) => imageUrl.trim() !== ""
      );
    }
    let imageUrl =
      imageOrVideoUrl && textType
        ? textType
        : cachedUserImages[
            Math.floor(Math.random() * cachedUserImages.length)
          ];
    if (!userImageCache[imageUrl]) {
      ...
      userImageCache[imageUrl] = {
        image: messageOptions,
        video: isVideo,
      };
    }
    quotedMessage =
      quotedMessage && quotedMessage.quoted?.key ? quotedMessage.quoted : quotedMessage || "";
    let messageContent;
    if (
      (((forceRandomImage && textType && global.style > 0) || !textType) &&
        /text|txt|nothing|amd|asta/.test(global.userImages)) ||
      imageOrVideoUrl == "text"
    ) {
      messageContent = {
        text: messageOptions.text || messageOptions.caption,
        ...messageOptions,
      };
    } else if (
      imageOrVideoUrl == "image" ||
      userImageCache[imageUrl].image
    ) {
      messageContent = {
        image: {
          url: imageUrl,
      ...
      imageOrVideoUrl == "video" ||
      userImageCache[imageUrl].video
    ) {
      messageContent = {
        video: {
          url: imageUrl,
    ...
    const additionalContext = (forceRandomImage && textType && global.style > 0)
      ? await amdBuffer(textType)
      : null;
    contextInfo = {
      ...(await Sock.contextInfo(
        Config.botname,
        quotedMessage && quotedMessage.senderName
    ...
    console.log("Error in sendUi() : ", error);
  }
  try {
    ...
    return Sock.sendMessage(recipientId, {
      text: messageOptions.text || messageOptions.caption,
      ...messageOptions,
    });
  }
