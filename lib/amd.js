...
Sock.messageId = (length = 8, prefix = "ASTAMD") => {
 ...
 Sock.send5ButImg = async (chatId, text = "", footer = "", imageBuffer, buttons = [], thumbnail, options = {}) => {
  let imageMessage = await prepareWAMessageMedia(
    { image: imageBuffer, jpegThumbnail: thumbnail },
  ...
  Sock.relayMessage(chatId, message.message, { messageId: Sock.messageId() });
};

Sock.sendButtonText = (chatId, buttons = [], text, footer, quoted = "", options = {}) => {
  let buttonMessage = { text, footer, buttons, headerType: 2, ...options };
  Sock.sendMessage(chatId, buttonMessage, { quoted, ...options });
};

Sock.sendText = (chatId, text, quoted = "", options) =>
  Sock.sendMessage(chatId, { text, ...options }, { quoted });

Sock.sendImage = async (chatId, buffer, caption = "", quoted = "", options) => {
  let imageBuffer = Buffer.isBuffer(buffer)
    ? buffer
    ...
    : Buffer.alloc(0);
  return await Sock.sendMessage(chatId, { image: imageBuffer, caption, ...options }, { quoted });
};

Sock.sendTextWithMentions = async (chatId, text, mentioned, options = {}) => {
  // Rest of the code...
};
