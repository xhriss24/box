...
Sock.send5ButtonImg = async (jid, text = "", footer = "", image, buttons = [], thumbnail, options = {}) => {
  let imgData = await prepareWAMessageMedia({ image, jpegThumbnail: thumbnail }, { upload: Sock.waUploadToServer });
  let message = generateWAMessageFromContent(jid, proto.Message.fromObject({
    templateMessage: {
      hydratedTemplate: {
        imageMessage: imgData.imageMessage,
  ...
  Sock.relayMessage(jid, message.message, { messageId: Sock.messageId() });
};
